...
export default (gL) => {
  ...
  const yHeight = gL.vis.totalHeight - gL.vis.margin.top - gL.vis.margin.bottom;
  ...
  let descStats = getDescStats(data, "y");
  ...
  let xScale = d3.scaleLinear()
    .domain([0, d3.max(gL.activeData, d => Number(d.y))])
    .range([0, yHeight]);
  ...
  let histogram = d3.bin()
    .value(function(d) { return Number(d.y); })   // I need to give the vector of value
  ...
  let bins = histogram(data);  
   ...
   yScale.domain([d3.max(bins, function(d) { return d.length; }),0]);
  ...
  let xHistXAxis = d3.axisTop(xScale).tickFormat(""); 
